...
{
       0,   247,   247,   256,   263,   272,   279,   286,   295,   315,
     337,   379,   423,   435,   445,   455,   468,   475,   485,   494,
     500,   526,   533,   540,   549,   558,   567,   577,   589,   597,
     607,   616,   623,   631,   639,   647,   655,   663,   671,   682,
     690,   700,   732,   775,   786,   813,   823,   844,   853,   872,
     881,   913,   922,   965,   983,  1001,  1013,  1023,  1107,  1116,
    1124,  1132,  1141,  1152,  1160,  1168,  1178,  1189
};
#endif
			...
			if(!ok)
			{
				...
				else	//check if declaration and definition are consistent
				{
					...
					if(!is_consistent)
					{
						...
						error_cnt++;
						error_print_line();
						fp3<<"function " + (yyvsp[-4].si)->getName() + "definition not consistent with declaration" <<endl<<endl;
					}
...
#line 469 "./parser.y"
                        {
				(yyval.si) = new SymbolInfo((yyvsp[-4].si)->getName()+"\n"+(yyvsp[-2].si)->getName()+(yyvsp[-1].si)->getName(), "NON_TERMINAL");
				print_line();
...
#line 701 "./parser.y"
			...
			else
			{
				...
				if(s->get_array())
				{
					error_cnt++;
					error_print_line();
					fp3<<(yyvsp[0].si)->getName()+" is an array"<<endl<<endl;
...
#line 882 "./parser.y"
			...
			//type conversion
			if((yyvsp[-2].si)->get_data_type() == "float" | (yyvsp[0].si)->get_data_type() == "float")
			{
...
#line 1024 "./parser.y"
		...
		if(s == NULL)
		{
			error_cnt++;
			error_print_line();
			fp3<<(yyvsp[-3].si)->getName() + " has not been declared"<<endl<<endl;
		...
		else //is declared
		{
			...
			if(f == NULL)
			{
				error_cnt++;
				error_print_line();
				fp3<<(yyvsp[-3].si)->getName() + " not a function"<<endl<<endl;
			}
			...
			else //is a function
			{
				...
				else //function defined
				{
					...
					if((int)arg_list.size() != f->getNumber_of_param())
					{
						error_cnt++;
						error_print_line();
						fp3<<(yyvsp[-3].si)->getName()+ " func parameters do not match with definition"<<endl<<endl;
					}
...
#line 1169 "./parser.y"
				...
				arg_list.push_back((yyvsp[-2].si)->get_data_type());
